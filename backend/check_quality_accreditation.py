#!/usr/bin/env python3
"""
Script to check if quality_accreditation table exists and verify its schema
"""
...
def main():
    try:
        ...
        # Check if quality_accreditation table exists
        print("\n🔍 Checking table: quality_accreditation")
        try:
            # Try to get one row from the table
            result = supabase.table('quality_accreditation').select("*").limit(1).execute()
            if hasattr(result, 'data'):
                ...
                all_result = supabase.table('quality_accreditation').select("*", count='exact').execute()
                ...
                if result.data and len(result.data) > 0:
                    ...
                    expected_columns = ['report_id', 'report_type', 'department', 'report_date', 'score', 'recommendations']
                    for col in columns:
