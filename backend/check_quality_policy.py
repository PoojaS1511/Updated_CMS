#!/usr/bin/env python3
"""
Script to check if quality_policy table exists and verify its schema
"""
...
def main():
    try:
        ...
        # Check if quality_policy table exists
        print("\n🔍 Checking table: quality_policy")
        try:
            # Try to get one row from the table
            result = supabase.table('quality_policy').select("*").limit(1).execute()
            if hasattr(result, 'data'):
                ...
                all_result = supabase.table('quality_policy').select("*", count='exact').execute()
                ...
                if result.data and len(result.data) > 0:
                    ...
                    expected_columns = ['policy_id', 'policy_name', 'responsible_department', 'compliance_status', 'last_review_date', 'next_due_date']
                    for col in columns:
