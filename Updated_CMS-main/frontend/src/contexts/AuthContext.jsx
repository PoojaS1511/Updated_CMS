  ...
  const updateUserState = useCallback(async (newSession) => {
    ...
    try {
      ...
      const userRole = newSession.user?.user_metadata?.role ||
                      newSession.user?.app_metadata?.role ||
                      (userEmail.endsWith('@college.edu') ? 'admin' : null);

      // For admin users, set admin role directly
      if (userRole === 'admin') {
        const adminUser = {
          ...newSession.user,
          role: 'admin',
      ...
      // First, check if this is a FACULTY (only for non-admin users)
      const { data: facultyData, error: facultyError } = await supabase
        .from('faculty')
